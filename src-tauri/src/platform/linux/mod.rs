...
impl GhProvider for LinuxGhProvider {
    ...
    fn ensure_required_scopes(&self, username: &str) -> AppResult<()> {
        ...
        let status = self.get_auth_status()?;
        let Some(account) = status.accounts.iter().find(|account| {
        ...
        if has_required_scopes(&account.scopes) {
            return Ok(());
        }
...
pub fn has_required_scopes(scopes: &[String]) -> bool {
    REQUIRED_GH_SCOPES.iter().all(|required_scope| {
        scopes
            .iter()
                ...
                for entry in entries {
                    ...
                    accounts.push(GhAccount {
                        ...
                        state: entry
                            ...
                            .and_then(Value::as_str)
                            .map(str::to_string),
                        scopes: parse_gh_account_scopes(entry),
                    });
                }
            }
        }
...
fn parse_gh_account_scopes(entry: &Value) -> Vec<String> {
    let Some(scopes) = entry.get("scopes") else {
        return Vec::new();
    };

    if let Some(scopes) = scopes.as_str() {
        return scopes
            .split(',')
            .map(str::trim)
...
pub fn open_path_with_system(path: &str) -> AppResult<()> {
    ...
    if output.status.success() {
        ...
        Err(AppError::Command(
            tr("system_open_path_failed_linux").into(),
        ))
    }
}
...
fn open_url_with_system(url: &str) -> AppResult<()> {
    ...
    if output.status.success() {
        ...
        Err(AppError::Command(
            tr("system_open_url_failed_linux").into(),
        ))
    }
}
...
mod tests {
    ...
    fn parses_gh_auth_status_json_with_login_field() {
        ...
        assert_eq!(parsed.accounts[0].scopes, vec!["gist", "read:org", "repo"]);
        assert!(!has_required_scopes(&parsed.accounts[0].scopes));
        assert_eq!(parsed.active_user.as_deref(), Some("keratolabs"));
    }

    #[test]
    fn parses_gh_auth_status_json_with_required_scopes() {
        let input = r#"{"hosts":{"github.com":[{"active":true,"login":"phricardo","scopes":"gist, read:org, repo, workflow"}]}}"#;
        let parsed = parse_gh_auth_status(input).unwrap();
        assert_eq!(parsed.accounts.len(), 1);
        assert!(has_required_scopes(&parsed.accounts[0].scopes));
    }
    ...
    #[test]
    fn parses_gh_auth_status_json_with_scope_array() {
        let input = r#"{"hosts":{"github.com":[{"active":true,"login":"phricardo","scopes":["gist","read:org","repo","workflow"]}]}}"#;
        let parsed = parse_gh_auth_status(input).unwrap();
        assert_eq!(
            parsed.accounts[0].scopes,
